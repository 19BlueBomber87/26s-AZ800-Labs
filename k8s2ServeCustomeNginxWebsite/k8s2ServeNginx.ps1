...
# ===================================================
# Step 1 - Create k8s control plane node and worker nodes
# ===================================================
# Note: For the Linux OS install Wizard to kick off, use at least 2GB.

#Create VM for control plane.  Keep control plane node's memory at 3 GB. 
# The Control Plane node will need a minimum 1700 MB.  
$k8sNodeName = "k8s"
$iso = "C:\ISO\ubuntu-26.04-live-server-amd64.iso"
New-Lab_VM -VMNames $k8sNodeName -HyperVSwitch Linux-Net -RAM 3GB -ISOPath $iso
Stop-VM -VMName $k8sNodeName -Force -Verbose *>&1
Set-VMFirmware -VMName $k8sNodeName -EnableSecureBoot Off -Verbose *>&1
Start-VM -VMName $k8sNodeName -Verbose *>&1
# Install Linux OS
# Keep control plane node's memory at 2GB.  The other VMs will need 2GB at boot time, putting the VM in a save state helps manage memory.  
Save-VM -VMName $k8sNodeName -Verbose *>&1

$k8sNodeName = "k8s-node01"
$iso = "C:\ISO\ubuntu-26.04-live-server-amd64.iso"
New-Lab_VM -VMNames $k8sNodeName -HyperVSwitch Linux-Net -RAM 2GB -ISOPath $iso
Stop-VM -VMName $k8sNodeName -Force -Verbose *>&1
Set-VMFirmware -VMName $k8sNodeName -EnableSecureBoot Off -Verbose *>&1
Start-VM -VMName $k8sNodeName -Verbose *>&1
# Install Linux OS

$k8sNodeName = "k8s-node02"
$iso = "C:\ISO\ubuntu-26.04-live-server-amd64.iso"
New-Lab_VM -VMNames $k8sNodeName -HyperVSwitch Linux-Net -RAM 2GB -ISOPath $iso
Stop-VM -VMName $k8sNodeName -Force -Verbose *>&1
Set-VMFirmware -VMName $k8sNodeName -EnableSecureBoot Off -Verbose *>&1
...
# ===================================================
# Step 2 - Configure k8s control plane node
# ===================================================
# Get IP of Linux nodes for SSH
# From each node figure out IP.  This lets us use SSH from a Desktop, so we can copy and paste configs.  
ip addr | grep eth
...
kubelet --version
# ===================================================
# Initialize The Cluster and configure calico network plugin on control plane node
# ===================================================
...
# This is the standard way to install Calico as your Kubernetes networking solution (CNI - Container Network Interface).
# Why do you need this?
# When you install Kubernetes (kubeadm, k3s, etc.), the control plane comes up, but pods cannot talk to each other yet. Kubernetes requires a separate networking plugin to:

# Assign IP addresses to pods
# Enable pod-to-pod communication across nodes
# Handle network policies (security rules between pods)
# Manage routing

# Calico is one of the most popular and powerful options for this.


# Install the Tigera Operator
# tigera-operator.yaml → This deploys the operator that manages Calico. It's the modern, recommended way to install Calico.
...
# IMPORTANT After a few minutes(5-10 mins) the status will change from Not Ready to Ready
...
# Pre‑pull the main Calico images the operator might use:
sudo ctr images pull quay.io/calico/node:v3.31.4
sudo ctr images pull quay.io/calico/cni:v3.31.4
sudo ctr images pull quay.io/calico/pod2daemon-flexvol:v3.31.4

# Pre‑pull all Calico images the operator might use:
# sudo ctr images pull quay.io/calico/node:v3.31.4
# sudo ctr images pull quay.io/calico/cni:v3.31.4
# sudo ctr images pull quay.io/calico/pod2daemon-flexvol:v3.31.4
# sudo ctr images pull quay.io/calico/apiserver:v3.31.4
# sudo ctr images pull quay.io/calico/typha:v3.31.4
# sudo ctr images pull quay.io/tigera/operator:v1.36.4


# Get Join command from control plane (run this on control plane and paste the output here to run on worker):
sudo kubeadm token create --print-join-command
# EXAMPLE TO RUN ON WORKER NODE(NOTE:USE SUDO!!): sudo kubeadm join 192.168.11.17:6443 --token z3dsf2.yvwfggorjpd9tswo --discovery-token-ca-cert-hash sha256:ea6f1ff344176d7667df10285945bca7eec1aeb8fa995cb85d15a5631a819be8


# Check Status when wating for "Not Ready" to change to "Ready"
kubectl describe node k8s-node01
kubectl describe node k8s-node02


# Make Sure calico pods are working
kubectl get pods -n calico-system -o wide | grep k8s-node03

# See the status of a calico pod
kubectl describe pod -n calico-system calico-node-sshnr



# On Control Plane Node Check calico pods
kubectl get pods -n kube-system -o wide | grep calico

# Verify the worker node joined
# Wait a few minutes and you will see node status change to "Ready" on control plane when worker joins.  
kubectl get nodes
watch kubectl get nodes
...
# Wait for pods ready in metallb-system.  
...
# Official Ingress‑NGINX deployment manifest for bare‑metal clusters
...
kubectl describe pod ingress-nginx-controller-74d74b7998-z9k4n -n ingress-nginx
...
data:
  customize.sh: |
    ...
    LP=$(grep -c ^processor /proc/cpuinfo)
    ...
    if [ -f "$TARGET_FILE" ]; then
      ...
      sed -i "s/Custom Heading Size and Font Type/Welcome to K8s <br>Computer Name: $ESC_HOST<br>OS Version: $ESC_OS<br>Date: $TODAY<br>CPU: $ESC_CPU<br>Logical Processor Count:$LP<br>IP: $ESC_IP/g" "$TARGET_FILE"
      echo "sed finished"
...
# To start over
kubectl delete deployment custom-static-site
...
kubectl scale deployment custom-static-site --replicas=21
# Watch Pods come up and see how they get distributed across nodes (if you have more than one worker):
kubectl get pods -w
kubectl get pods -o wide
# Test from any machine on your LAN: 192.168.11.200.  Note Use "New Private Window" in browser for best results to see pod info change on refresh.
kubectl scale deployment custom-static-site --replicas=31
...
# See how big you can make it! If you make to many, host will lock up!! <@:D
kubectl scale deployment custom-static-site --replicas=50
