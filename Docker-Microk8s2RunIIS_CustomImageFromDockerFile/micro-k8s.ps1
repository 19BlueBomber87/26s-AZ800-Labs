...
Install-WindowsFeature -Name Containers -IncludeAllSubFeature -IncludeManagementTools -Restart -Verbose *>&1
...
# ===============================================================================
$controlPlaneIP = "192.168.11.25" # UPDATE IP TO YOUR LINUX BOXES IP!!
...
$controlPlaneIP = "192.168.11.25"
http://192.168.11.25:8080/
...
# Copy ca.crt to home directory and share
sudo cp /var/snap/microk8s/current/certs/ca.crt ~/ca.crt
sudo python3 -m http.server --directory ~ 8081
# WINDOWS NODE Download and register cert to "micro-k8s-nodes"
mkdir "c:\certs"
#WINDOWS NODE Download config to "micro-k8s-node01"
$linuxIP = "192.168.11.25" # UPDATE IP!!
...
Get-ChildItem Cert:\LocalMachine\Root | ? -Property Subject -like *10.152.183.1*
...
#Note it takes a few moments for Status to go from 'Not Ready' to 'Ready'
...
# ===============================================================================
# It can take 30 mins at ≈ 50 Mbps
...
# Remove them and watch them come back
...
POD=iis-simple-5bc5db9fcb-7z8nl
# Use your Pod name
...
$linuxIP = "192.168.11.25"
# Check website by port
http://192.168.11.25:30080

#Scale Deploymnet
sudo microk8s kubectl scale deployment/iis-simple --replicas=9
...
#Check metallb.  May take a few minutes
...
watch sudo microk8s kubectl get pods -o wide
