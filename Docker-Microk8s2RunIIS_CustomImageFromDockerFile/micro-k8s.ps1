...
#                   [Link] -> https://github.com/19BlueBomber87/26s-AZ800-Labs/blob/main/Microk8s2RunIIS_CustomImage/BuildAndPushImagefromDockerFile.ps1
# ===============================================================================



# ===============================================================================
# Step 1 -Create "k8s Windows Worker Node" Image With a Docker File
...
# ===============================================================================
# Step 2 -Create Control Plane Node and Windows Worker Nodes virtual machines
...
New-Lab_VM -VMNames "micro-k8s-node01", "micro-k8s-node02", "micro-k8s-node03", "micro-k8s-node04" -HyperVSwitch Linux-Net -GeneralizedImageCore
$microK8sNodes = Get-VM | Where-Object -Property VMName -Like *micro-k8s-node*
foreach($node in $microK8sNodes){
...
$VMs = Get-VM | Where-Object -Property VMName -Like *micro-k8s-node*
foreach($VM in $VMs){
...
# DNS allows containers (pods) to find each other by name instead of IP.
...
$linuxIP = "192.168.11.44" # UPDATE IP!!
...
ipconfig
# Update c:\calicowindows\kubernetes\kubelet-service.ps1 ->IP address and [string]$InterfaceName="vEthernet (Ethernet)"
...
$linuxIP = "192.168.11.43"
# Check website by port
http://192.168.11.43:30080
...
sudo microk8s kubectl describe pod iis-simple-6f577b79d8-87pjh
#Scale Deploymnet
sudo microk8s kubectl scale deployment/iis-simple --replicas=21
