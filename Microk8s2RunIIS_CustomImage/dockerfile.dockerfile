...
# Disable auto-start of w3svc ->  It gives you full control over when IIS starts 
#sc = Windows Service Control utility -> w3svc = IIS World Wide Web Publishing Service (the core IIS web server service) -> start= demand = sets the service to manual startup instead of automatic
# So this command changes IIS so that: It does NOT start automatically when the container starts.  It will only start when explicitly triggered
